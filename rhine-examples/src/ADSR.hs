...
data ADSR time s = ADSR
  { a :: Diff time
  -- ^ The attack time (for how long the level increases)
  , d :: Diff time
  -- ^ The decay time (for how long the level decreases)
  , s :: s
  -- ^ The sustain level (a 'Fractional' between 0 and 1)
  , r :: Diff time
  -- ^ The release time (how long the level needs to decay)
  }
...
linearly ::
  ...
  -- | The final amplitude
  amplitude ->
  -- | How far overdue the interpolation already is
  Diff time ->
...
decay ::
  ...
  -- | ...the sustain level.
  amplitude ->
  -- | How far overdue the decay period already is.
  Diff time ->
  Behaviour (ExceptT (Diff time) m) time amplitude
decay d = linearly d 1

{- HLINT ignore sustain "Eta reduce" -}
