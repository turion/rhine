...
To compose two `Rhine`s in parallel we need a shared `Schedule` between those two. To share a `Schedule` between two `Rhine`s we need to first construct a `RhineParallelAndSchedule` out of one `Rhine` and one `Schedule`. To do this we can use the constructor `RhineParallelAndSchedule` or the aliases `||@` or `++@`. Then we need to combine that resulting `RhineParallelAndSchedule` with another `Rhine` using `@||` or `@++`
...
`@||` is defined using `||||`. This is the implementation:
...
`@++` is also defined the same way except it replaces `||||` with `++++`. This is the implementation:
