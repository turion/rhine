...
(^>>>)
  ...
  -> SN m cl      b c
  -> SN m cl a      c
f ^>>> SN {getSN} = SN $ lmap (fmap (fmap f)) getSN
...
(****)
  ...
  -> SN m cl     c      d
  -> SN m cl (a, c) (b, d)
SN sn1 **** SN sn2 = SN $ arr (\(time, tag, mac) -> ((time, tag, fst <$> mac), (time, tag, snd <$> mac))) >>> (sn1 *** sn2) >>> arr (\(mb, md) -> (,) <$> mb <*> md)
