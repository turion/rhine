...
-- rhine
import FRP.Rhine.Clock (Clock (runClock), RunningClockInit)
  ...
  testGroup
    ...
    , testGroup
        ...
        [ testCase "chronological ticks" $ do
            let clA = FixedStep @5
                clB = FixedStep @3
                (runningClockA, _) = runSchedule (runClock clA :: RunningClockInit (Schedule Integer) Integer ())
                (runningClockB, _) = runSchedule (runClock clB :: RunningClockInit (Schedule Integer) Integer ())
    ...
    , testGroup
        ...
        [ testCase "chronological ticks" $ do
            let
              (runningClock, _time) = runSchedule (runClock $ ParallelClock (FixedStep @5) (FixedStep @3) :: RunningClockInit (Schedule Integer) Integer (Either () ()))
