...
inferenceSimpleParam :: Rhine (GlossConcT IO) (LiftClock IO GlossConcT Busy) (Temperature, (Sensor, Pos)) Result
-- FIXME abstract that bracket
inferenceSimpleParam = hoistClSF sampleIOGloss inferenceBehaviour @@ liftClock Busy
 where
  inferenceBehaviour :: (MonadIO m, MonadDistribution m, Diff td ~ Double) => BehaviourF m td (Temperature, (Sensor, Pos)) Result
  inferenceBehaviour = proc (temperature, (measured, latent)) -> do
...
inferenceDirichletConstant :: Rhine (GlossConcT IO) (LiftClock IO GlossConcT Busy) (Temperature, (Sensor, Pos)) Result
-- FIXME abstract that bracket
...
mainRhineDirichletConstant = userTemperature @@ glossClockUTC GlossEventClockIO >-- keepLast initialTemperature -@- glossConcurrently --> modelRhine >-- keepLast (initialTemperature, (zeroVector, zeroVector)) -@- glossConcurrently --> inferenceDirichletConstant >-- keepLast Result{temperature = initialTemperature, measured = zeroVector, latent = zeroVector, particles = []} -@- glossConcurrently --> visualisationRhine
...
inferenceDirichletElastic :: Rhine (GlossConcT IO) (LiftClock IO GlossConcT Busy) (Temperature, (Sensor, Pos)) Result
-- FIXME abstract that bracket
