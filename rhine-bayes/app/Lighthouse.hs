...
type Sensor = DigitsPrecision 2 Double
...
noise :: MonadDistribution m => Behaviour m td Sensor
noise = let stdDev = 0.01 in DigitsPrecision <$> whiteNoise stdDev
...
generativeModel = proc latent -> do
  let (distanceX, distanceY) = latent ^-^ lighthouse
  noiseNow <- noise -< ()
  returnA -< DigitsPrecision $ distanceX / distanceY
  -- returnA -< (distanceX / distanceY) + noiseNow
...
visualisation = proc result@Result { measured, latent, particles } -> do
  constMCl clearIO -< ()
  -- arrMCl $ liftIO . print -< result
  arrMCl paintIO -< color yellow $ line $ double2FloatTuple <$> [lighthouse, lighthouse ^+^ (300 *^ (getDigitsPrecision measured, 1))] -- (measured, 0.3, red)
...
mainRhine = const stdDev ^>>@ modelRhine >-- keepLast (stdDev, (0, zeroVector)) -@- glossConcurrently --> inference >-- keepLast Result { measured = 0, latent = zeroVector, particles = []} -@- glossConcurrently --> visualisationRhine
