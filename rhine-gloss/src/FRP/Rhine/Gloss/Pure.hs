...
instance Clock GlossM GlossClock where
  ...
  runClock = constM (GlossM (skip >> lift ask)) >>> (sumN *** Category.id)
  {-# INLINE runClock #-}
...
flowGloss GlossSettings {..} rhine =
  ...
  where
    ...
    worldAutomaton = AutomatonWriter.runWriterS $ AutomatonReader.runReaderS $ hoistS (runSkipT . unGlossM) $ eraseClock rhine
